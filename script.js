...
async function init() {
    try {
       ...
       defaultSegments = [
    { text: 'Caneta', emoji: '✏️', emojiSize: 26, color: '#dd86ca' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#cc81eb' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#c16cce' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#66005c' },
    { text: 'Mochila', emoji: '🎒', emojiSize: 26, color: '#dd86ca' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#cc81eb' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#c16cce' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#66005c' },
    { text: 'Porta-chaves', emoji: '🔑', emojiSize: 26, color: '#dd86ca' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#cc81eb' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#c16cce' },
    { text: 'Voucher', emoji: '💗', emojiSize: 26, color: '#66005c' }
];
...
function renderWheel() {
    ...
    wheel.segments.forEach((segment, index) => {
        ...
        const isWhite = segment.color === '#FFFFFF';
        ctx.fillStyle = isWhite ? '#9B6B7F' : '#FFFFFF';
        ...
        if (segment.emoji) {
            ...
            ctx.fillText(segment.emoji, radius - 60, 0);
            
            ctx.font = `600 13px Poppins`;
            ctx.fillText(segment.text, radius - 95, 3);
        } else {
            ctx.font = `600 14px Poppins`;
            ctx.fillText(segment.text, radius - 60, 3);
        }
        ctx.restore();
    });
    ...
    wheels[currentWheelId].segments.forEach((s, i) => {
        ...
        div.innerHTML = `
            ...
            <div class="segment-controls">
                <input type="text" value="${s.emoji}" placeholder="Emoji" maxlength="2" oninput="updateSeg(${i}, 'emoji', this.value)">
                <input type="text" value="${s.text}" placeholder="Texto" oninput="updateSeg(${i}, 'text', this.value)">
                <div class="emoji-size-control">
                    <label>Tamanho:</label>
                    <input type="range" min="16" max="40" value="${s.emojiSize || 26}" class="emoji-size-slider" onchange="updateSeg(${i}, 'emojiSize', parseInt(this.value))">
                    <span class="size-value">${s.emojiSize || 26}</span>
                </div>
            </div>
...
function updateSeg(idx, field, val) {
    if (field === 'emojiSize') {
        ...
        if (sizeSpans[idx]) sizeSpans[idx].textContent = val;
    } else {
...
function addToHistory(winner) {
    ...
    if (wheels[currentWheelId].history.length > 20) wheels[currentWheelId].history.pop();
    updateHistory();
}
...
function updateHistory() {
    ...
    h.innerHTML = items.map(i => 
        `<div class="history-item">${i.emoji} <strong>${i.text}</strong></div>`
    ).join('');
}

function showWinner(text, color, emoji) {
    const div = document.createElement('div');
    const textColor = color === '#FFFFFF' ? '#9B6B7F' : 'white';
    ...
    div.style.cssText = `
        ...
        border: 1px solid rgba(221, 134, 202, 0.15);
        z-index: 10000;
        text-align: center;
    ...
    div.innerHTML = `
        <div style="font-size: 70px; margin-bottom: 20px;">${emoji}</div>
        <h2 style="font-size: 32px; color: ${color}; margin: 0; font-weight: 700; letter-spacing: -0.5px;">${text}</h2>
    `;
